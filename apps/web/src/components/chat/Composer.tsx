...
export function Composer({ connectionState }: ComposerProps) {
  const { sendTextMessage, sendTyping } = useChat();
  const [draft, setDraft] = useState('');
  const typingOffTimer = useRef<ReturnType<typeof setTimeout> | null>(null);

  const disabled = connectionState === 'disconnected';
  ...
  return (
    <footer className="flex items-end gap-2 border-t border-[#e9edef] bg-[#f0f2f5] px-4 py-3">
      <button
        type="button"
        disabled
        className="shrink-0 rounded-full p-2 text-[#54656f] opacity-50"
        aria-label="Attach file (coming soon)"
        title="Attachments in a future update"
      >
